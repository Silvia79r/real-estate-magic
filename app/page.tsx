        ...
        {activeTab === 'home' ? (
          <div className="space-y-8 animate-in fade-in">
            ...
            <div onClick={() => setActiveTab('shop')} className="bg-slate-900 p-8 rounded-[2.5rem] text-white shadow-xl cursor-pointer active:scale-95 transition-all mt-4 relative overflow-hidden">
               <div className="relative z-10">
                 ...
                 <p className="text-sm opacity-70 leading-relaxed font-medium text-slate-300">Scegli un pacchetto per il tuo team professionale.</p>
               </div>
          ...
          <div className="space-y-6 animate-in slide-in-from-bottom-6">
            ...
            <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 text-center">
              {!isDone ? (
                <>
                  <div className="mb-8 p-6 bg-blue-50 rounded-3xl text-left border border-blue-100 shadow-inner">
                  ...
                  <label className="relative aspect-square border-2 border-dashed border-slate-200 rounded-[2.5rem] bg-slate-50 flex flex-col items-center justify-center mb-8 overflow-hidden cursor-pointer active:bg-slate-100 transition-all shadow-inner">
                    ...
                    {isProcessing ? (
                      <div className="flex flex-col items-center gap-4">
                        ...
                        <p className="text-blue-600 font-black text-xs uppercase animate-pulse">Analisi in corso...</p>
                      </div>
                    ) : selectedImage ? (
                      <img src={selectedImage} alt="Preview" className="w-full h-full object-cover animate-in fade-in" />
                    ) : (
                  ...
                  <button 
                    ...
                    className={`w-full py-7 rounded-[2rem] font-black text-lg shadow-xl flex items-center justify-center gap-4 transition-all active:scale-95 ${!selectedImage ? 'bg-slate-200 text-slate-400 cursor-not-allowed' : 'bg-blue-600 text-white shadow-blue-200'}`}
                  >
                  ...
                  <div className="w-full space-y-4">
                    <button className="w-full bg-slate-900 text-white p-6 rounded-[2rem] flex items-center justify-between shadow-lg active:scale-95 transition-all">
                      <div className="flex items-center gap-4">
                        ...
                        <span className="font-black text-sm tracking-tight uppercase tracking-widest italic">Portale (4:3)</span>
                      </div>
                    ...
                    <button className="w-full bg-white border-4 border-slate-100 text-slate-900 p-6 rounded-[2rem] flex items-center justify-between shadow-sm active:scale-95 transition-all">
                      <div className="flex items-center gap-4">
                        ...
                        <span className="font-black text-sm tracking-tight uppercase tracking-widest italic">Social (4:5)</span>
                      </div>
                      <Download className="w-6 h-6 text-purple-500" />
                    </button>
                  </div>

                  <button onClick={() => {setIsDone(false); setSelectedImage(null);}} className="mt-12 text-slate-400 font-black text-[10px] uppercase tracking-[0.2em] border-b-2 border-slate-200 pb-1">
                    Nuova Elaborazione
                  </button>
                </div>
              )}
