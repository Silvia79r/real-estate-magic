    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-900">
      <header className="bg-white border-b border-slate-200 px-4 py-4 sticky top-0 z-50">
        <div className="max-w-md mx-auto flex items-center justify-between">
          ...
          <button onClick={() => setActiveTab('shop')} className="bg-blue-50 px-4 py-2 rounded-full flex items-center gap-2 border border-blue-100 shadow-sm">
        ...
        {activeTab === 'home' ? (
          <div className="space-y-8 animate-in fade-in">
            ...
            <div onClick={() => setActiveTab('shop')} className="bg-slate-900 p-8 rounded-[2.5rem] text-white shadow-xl cursor-pointer active:scale-95 transition-all mt-4 text-center">
               <h3 className="text-xl font-black mb-2 uppercase tracking-tight italic text-blue-400">Ricarica Crediti</h3>
               <p className="text-sm opacity-80 leading-relaxed font-medium text-slate-300">Scegli un pacchetto professionale per il tuo team.</p>
            </div>
          </div>
        ) : (
              ...
              {!isDone ? (
                <>
                  ...
                  <button 
                    ...
                    className={`w-full py-7 rounded-[2rem] font-black text-lg shadow-xl flex items-center justify-center gap-4 transition-all active:scale-95 ${!selectedImage ? 'bg-slate-200 text-slate-400' : 'bg-blue-600 text-white shadow-blue-200'}`}
                  >
                  ...
                  <div className="w-full space-y-4">
                    <button className="w-full bg-slate-900 text-white p-6 rounded-[2rem] flex items-center justify-between shadow-lg active:scale-95 transition-all">
                      <div className="flex items-center gap-4">
                        ...
                        <span className="font-black text-sm tracking-tight uppercase tracking-widest">Portale (4:3)</span>
                      </div>
                    ...
                    <button className="w-full bg-white border-4 border-slate-100 text-slate-900 p-6 rounded-[2rem] flex items-center justify-between shadow-sm active:scale-95 transition-all">
                      <div className="flex items-center gap-4">
                        ...
                        <span className="font-black text-sm tracking-tight uppercase tracking-widest">Social (4:5)</span>
                      </div>
