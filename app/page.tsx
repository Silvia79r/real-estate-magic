...
export default function RealEstateApp() {
  ...
  const startAiMagic = () => {
    ...
    // Simuliamo i 3 secondi che l'AI impiega per rispondere
    setTimeout(() => {
      setIsProcessing(false);
      setIsDone(true);
      setCredits(prev => prev - 1); // Scala il credito automaticamente
      ...
      <header className="bg-white border-b border-slate-200 px-4 py-3 sticky top-0 z-50">
        <div className="max-w-md mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="bg-blue-50 px-3 py-1 rounded-full flex items-center gap-2 border border-blue-100 transition-all">
      ...
      <main className="flex-1 p-4 max-w-md mx-auto w-full">
          ...
          <div className="aspect-[4/5] border-2 border-dashed border-slate-200 rounded-[2rem] flex flex-col items-center justify-center bg-slate-50 relative overflow-hidden">
            {isProcessing ? (
              <div className="flex flex-col items-center gap-4 animate-pulse">
                ...
                <p className="text-blue-600 font-bold uppercase tracking-widest text-xs">L'AI sta creando la magia...</p>
              </div>
            ) : isDone ? (
              <div className="flex flex-col items-center gap-4 text-emerald-500 animate-in zoom-in">
              ...
              <div className="text-center p-6 cursor-pointer" onClick={() => document.getElementById('file-upload')?.click()}>
                ...
                <p className="text-slate-600 font-bold uppercase text-xs">Carica l'immobile</p>
        ...
        <button 
          ...
          className={`w-full py-6 rounded-[2rem] font-black shadow-xl flex items-center justify-center gap-3 transition-all active:scale-95 ${
            isProcessing ? 'bg-slate-100 text-slate-400' : 'bg-blue-600 text-white hover:bg-blue-700'
          }`}
        >
          {isProcessing ? (
            <>
              <Loader2 className="w-6 h-6 animate-spin" />
              <span>ELABORAZIONE IN CORSO...
